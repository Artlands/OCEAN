...
run_destroy_for_host() {
    ...
    build_env_args env_args BR VNI CLEAN_ALL_TAPS
    build_remote_env_prefix remote_env_prefix BR VNI CLEAN_ALL_TAPS
    ...
    if [[ "$SSH_USER" == "root" ]]; then
        printf -v remote_cmd 'cd %q && env%s bash %q %q %q' "$REMOTE_REPO_ROOT" "$remote_env_prefix" "$DESTROY_SCRIPT_REL" "$num_vms" "$host_id"
    else
        printf -v remote_cmd 'cd %q && sudo env%s bash %q %q %q' "$REMOTE_REPO_ROOT" "$remote_env_prefix" "$DESTROY_SCRIPT_REL" "$num_vms" "$host_id"
    fi

    echo "==> host_id=${host_id} host_ip=${host_ip}"
    if [[ "$RUN_LOCAL" == "1" ]] && is_local_host_id "$host_id"; then
        echo "    running locally: ${DESTROY_SCRIPT_REL}"
        (cd "$REPO_ROOT" && "${local_prefix[@]}" env "${env_args[@]}" bash "$DESTROY_SCRIPT_REL" "$num_vms" "$host_id")
    else
        echo "    running via ssh ${SSH_USER}@${host_ip}:${SSH_PORT}: ${DESTROY_SCRIPT_REL}"
...
main() {
    ...
    for target in "${selected_hosts[@]}"; do
        run_destroy_for_host "$num_vms" "$target"
    done
}
