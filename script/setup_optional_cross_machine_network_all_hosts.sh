...
DESTROY_FIRST="${DESTROY_FIRST:-1}"
DESTROY_SCRIPT_REL="script/destroy_optional_cross_machine_network.sh"
SETUP_SCRIPT_REL="script/setup_optional_cross_machine_network.sh"
...
Environment:
  ...
  DESTROY_FIRST=1  # destroy the existing VM network on each host before setup (default)
USAGE
}
...
run_host_script() {
    ...
    local num_vms="$2"
    local host_id="$3"
    local require_root="$4"
    shift 4

    local host_ip="${HOST_IPS[$host_id]}"
    local remote_env_prefix remote_cmd
    local -a env_args=()
    local -a local_prefix=()

    build_env_args env_args "$@"
    build_remote_env_prefix remote_env_prefix "$@"

    if [[ "$require_root" == "1" && $(id -u) -ne 0 ]]; then
        local_prefix=(sudo)
    fi

    if [[ "$SSH_USER" == "root" || "$require_root" != "1" ]]; then
        printf -v remote_cmd 'cd %q && env%s bash %q %q %q' "$REMOTE_REPO_ROOT" "$remote_env_prefix" "$script_rel" "$num_vms" "$host_id"
    else
        printf -v remote_cmd 'cd %q && sudo env%s bash %q %q %q' "$REMOTE_REPO_ROOT" "$remote_env_prefix" "$script_rel" "$num_vms" "$host_id"
    fi

    if [[ "$RUN_LOCAL" == "1" ]] && is_local_host_id "$host_id"; then
        echo "    running locally: ${script_rel}"
        (cd "$REPO_ROOT" && "${local_prefix[@]}" env "${env_args[@]}" bash "$script_rel" "$num_vms" "$host_id")
    else
        echo "    running via ssh ${SSH_USER}@${host_ip}:${SSH_PORT}: ${script_rel}"
...
run_setup_for_host() {
    ...
    if [[ "$DESTROY_FIRST" == "1" ]]; then
        run_host_script "$DESTROY_SCRIPT_REL" "$num_vms" "$host_id" 1             BR VNI CLEAN_ALL_TAPS
    fi
    run_host_script "$SETUP_SCRIPT_REL" "$num_vms" "$host_id" 1         DEV BR VNI VXLAN_PORT MCAST VXLAN_HOSTS_FILE VXLAN_PEERS BR_ADDR LOCAL_IP CLEAN_ALL_TAPS
}
...
main() {
    ...
    for target in "${selected_hosts[@]}"; do
        run_setup_for_host "$num_vms" "$target"
    done
}
