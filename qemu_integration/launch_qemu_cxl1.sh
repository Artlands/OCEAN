...
GUEST_IP=${GUEST_IP:-192.168.100.11}
GUEST_HOSTNAME=${GUEST_HOSTNAME:-node1}
...
elif [[ -f "${IMAGES_DIR}/qemu1.qcow2" ]]; then
    disk_image="${IMAGES_DIR}/qemu1.qcow2"
elif [[ -f "${IMAGES_DIR}/qemu1.img" ]]; then
    disk_image="${IMAGES_DIR}/qemu1.img"
elif [[ -f ./qemu1.qcow2 ]]; then
    disk_image=./qemu1.qcow2
elif [[ -f ./qemu1.img ]]; then
    disk_image=./qemu1.img
else
    echo "Unable to locate a disk image for host 1." >&2
    echo "Set DISK_IMAGE explicitly or create ${IMAGES_DIR}/qemu1.qcow2" >&2
    exit 1
fi
...
export CXL_HOST_ID=${CXL_HOST_ID:-1}
